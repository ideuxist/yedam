	...
	public static void createAccount() {
		...
		while (true) {
			if (accMoney <= 100000) {
				...
				System.out.println("입금액은 100000원을 초과할 수 없습니다");
...
//출금 메소드
	public static void withdraw() {
		...
		int awd = scn.nextInt();
		int checkCnt = 0; //조회가 됐는지 체크여부 변수
		for(int i=0; i<banks.length;i++) {
			if (banks[i] != null && banks[i].getAccNo().equals(ano));{//계좌번호가 조회되었을때
			checkCnt=1;
			int currAmt = banks[i].getMoney(); 
	...
	if (checkCnt==1) {
		...
		System.out.println("출금한도액을 초과했습니다.");
	} else {
